 ...
 */
function resolveGlmTier(contextTokens, outputTokens) {
 ...
 * - low: context <= 512k
 */
function resolveMinimaxTier(contextTokens) {
    if (contextTokens > 512_000)
        return 'high';
    return 'low';
}
function resolveTier(provider, pricing, contextTokens, outputTokens) {
    if (provider === 'glm') {
 ...
 */
export function parseModelId(rawModelId) {
...
function providerToLimitsKey(provider) {
 ...
 */
export function getContextLimit(rawModelId, claudeCodeCtxSize) {
    const parsed = parseModelId(rawModelId);
    if (claudeCodeCtxSize)
        return claudeCodeCtxSize;
    // An explicit `[Nk]`/`[Nm]` suffix overrides the built-in table for any provider.
    if (parsed.ctxOverride)
        return parsed.ctxOverride;
    ...
    if (!providerLimits)
        return 200_000;
    if (typeof providerLimits === 'number')
        return providerLimits;
    if (parsed.modelId in providerLimits) {
        return providerLimits[parsed.modelId];
    }
    const values = Object.values(providerLimits);
 ...
 */
export function getModelPrice(rawModelId, contextTokens, outputTokens) {
    const parsed = parseModelId(rawModelId);
 ...
 */
export function calculateCost(rawModelId, inputUncachedTokens, cacheReadTokens, cacheCreationTokens, outputTokens, contextTokens) {
