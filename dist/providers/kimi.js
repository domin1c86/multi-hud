...
export class KimiProvider extends BaseProvider {
     ...
     */
    async getQuotas() {
        if (!this.config.apiKey || !this.config.apiKey.startsWith('sk-kimi'))
            return null;
        try {
    ...
    async fetchUsage() {
        const init = { headers: { 'User-Agent': 'KimiCLI/1.6' } };
        try {
            return await this.fetchJson('/usages', init, CODING_BASE_URL);
        }
        catch (err) {
            if (String(err).includes('404')) {
                return await this.fetchJson('/usage', init, CODING_BASE_URL);
            }
            throw err;
        }
    }
...
function windowName(item) {
    ...
    if (unit === 'HOUR' && dur === 5)
        return '5h';
    if ((unit === 'HOUR' && dur === 24) || (unit === 'DAY' && dur === 1))
        return '24h';
    if ((unit === 'DAY' && dur === 7) || unit === 'WEEK')
        return '7d';
    if (unit === 'DAY' && dur === 30)
        return '30d';
    ...
    if (label.includes('5h') || label.includes('5-hour') || label.includes('5 hour'))
        return '5h';
    if (label.includes('week') || label === '7d')
        return '7d';
    if (label.includes('month') || label === '30d')
        return '30d';
    if (label.includes('day') || label === '24h')
        return '24h';
    return null;
}
function percentage(used, limit, explicit) {
    if (explicit != null)
        return explicit;
...
function resetsAt(at, resetIn) {
    ...
    if (resetIn != null)
        return new Date(Date.now() + resetIn * 1000);
    return undefined;
}
